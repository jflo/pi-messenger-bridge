...
### Added
- `playerSafeTools` config option: a list of custom tool names non-admins are allowed to call, in addition to pi's fixed read-only tools. Admins now get every custom tool a project registers automatically; non-admins get none by default (fail closed) unless a project opts a tool in here
