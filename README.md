...
```json
{
  ...
  "admins": ["telegram:789"],
  "playerSafeTools": ["set_my_character_name"],
  "autoConnect": true,
  "showWidget": true,
  "debug": false
}
...
Tool access applies to **every** tool, not just pi's own built-ins — including custom tools a project registers via `pi.registerTool()` in its own `.pi/extensions/`. Admins get every registered tool automatically. Non-admins get none of them by default (fail closed) unless a project explicitly opts a tool in via `playerSafeTools` (a list of tool names) — since pi has no built-in notion of a tool being "safe" for a non-admin caller, this bridge has no way to guess which of a project's custom tools are appropriate to expose without the project saying so.
